...
public class AB {
	...
	public String exibirArvorePreOrdem(){
		...
		if (p.esq != null){listaArvore = listaArvore + "(" + exibirArvorePreOrdem(p.esq)+")";}
		if (p.dir != null){listaArvore = listaArvore + "(" + exibirArvorePreOrdem(p.dir)+ ")";}
		...
		while (tipodeOperacao != null){//inicio do loop
			...
			if (testeOperador){JOptionPane.showMessageDialog(null,"Entrada incorreta ");
				...
				if(tipodeOperacao.length() == 1 && tipodeOperacao.charAt(0) == '4'){//impressao
					...
					if(testeParametro){
						JOptionPane.showMessageDialog(null,"A árvore aceita apenas inteiros"," Arvore B ",JOptionPane.ERROR_MESSAGE);
					}
					else{
